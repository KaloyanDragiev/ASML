  ...
  if ((A->size[0] == 0) || (A->size[1] == 0)) {
    ...
    for (q = 0; q < m; q++) {
      ...
      if (q + 1 <= nct) {
        ...
        if (nrm > 0.0) {
          ...
          if (fabs(b_s->data[q]) >= 1.0020841800044864E-292) {
            xscal(nmq, 1.0 / b_s->data[q], b_A, qq);
          } else {
            ns = qq + nmq;
            for (k = qq; k < ns; k++) {
      ...
      for (k = qp1; k <= p; k++) {
        ns = q + n * (k - 1);
        if (apply_transform) {
          nrm = xdotc(nmq, b_A, qq, b_A, ns + 1);
          nrm = -(nrm / b_A->data[q + b_A->size[0] * q]);
          xaxpy(nmq, nrm, qq, b_A, ns + 1);
        }

        e->data[k - 1] = b_A->data[ns];
      }
      ...
      if (q + 1 <= nrt) {
        ...
        if (nrm == 0.0) {
          ...
          if (fabs(e->data[q]) >= 1.0020841800044864E-292) {
            if (pmq >= 1) {
              cblas_dscal((int)pmq, 1.0 / e->data[q], &e->data[q + 1], (int)1);
            }
          } else {
            ns = q + pmq;
            for (k = qp1; k <= ns + 1; k++) {
    ...
    for (q = nct; q >= 1; q--) {
      ...
      if (b_s->data[q - 1] != 0.0) {
        for (k = qp1; k <= n; k++) {
          ns = q + n * (k - 1);
          nrm = xdotc(nmq, U, qq + 1, U, ns);
          nrm = -(nrm / U->data[qq]);
          xaxpy(nmq, nrm, qq + 1, U, ns);
        }
    ...
    for (q = p; q >= 1; q--) {
      if ((q <= nrt) && (e->data[q - 1] != 0.0)) {
        ...
        for (k = qp1; k <= p; k++) {
          qq = (q + p * (k - 1)) + 1;
          nrm = xdotc(pmq, V, ns, V, qq);
          nrm = -(nrm / V->data[ns - 1]);
          xaxpy(pmq, nrm, ns, V, qq);
        }
      }
    ...
    while ((m + 1 > 0) && (pmq < 75)) {
      ...
      while (!(exitg1 || (ii == 0))) {
        ...
        if ((nrm <= 2.2204460492503131E-16 * (fabs(b_s->data[ii - 1]) + fabs
              (b_s->data[ii]))) || (nrm <= 1.0020841800044864E-292) || ((pmq >
      ...
      if (ii == m) {
        ...
        while ((!exitg1) && (qq >= ii)) {
          ...
          if (qq == ii) {
            ...
            nrm = 0.0;
            if (qq < m + 1) {
              nrm = fabs(e->data[qq - 1]);
            }

            if (qq > ii + 1) {
              nrm += fabs(e->data[qq - 2]);
            }

            rt = fabs(b_s->data[qq - 1]);
      ...
      switch (qq) {
       ...
       default:
        ...
        qp1 = ii + 1;
        while ((ii + 1 < nctp1 + 1) && (b_s->data[ii] < b_s->data[qp1])) {
          rt = b_s->data[ii];
